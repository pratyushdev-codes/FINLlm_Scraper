...
async def crawl_data():
    """
    Main function to crawl  data from the website.
    """
    # Initialize configurations
    browser_config = get_browser_config()
    llm_strategy = get_llm_strategy()
    session_id = "venue_data_session"
    ...
    async with AsyncWebCrawler(config=browser_config) as crawler:
        while True:
            ...
            data, no_results_found = await fetch_and_process_page(
                crawler,
                page_number,
                BASE_URL,
                CSS_SELECTOR,
    ...
    if all_data:
        save_data_to_csv(all_data, "complete_data.csv")
        print(f"Saved {len(all_data)} venues to 'complete_data.csv'.")
    else:
        print("No data was found during the crawl.")
